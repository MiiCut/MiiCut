use crate::{
    ...
    shapes::{
        shapes::{DrawableShape, MoveShapesAction},
...
pub enum MoveAction<S: DrawableShape> {
    Shapes(MoveShapesAction<S>),
    Helpers(MoveHelpersAction),
}
impl<S: DrawableShape> Action for MoveAction<S> {
    fn undo(&self, pools: &mut ShapesPool<S>) {
        match self {
...
// #[derive(Clone, Debug)]
// pub struct Pools<S: DrawableShape> {
//     pub shapes: ShapesPool,
//     pub helpers: HelpersPool,
// }
// impl<S: DrawableShape> Pools<S: DrawableShape> {
