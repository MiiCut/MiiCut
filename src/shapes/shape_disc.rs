...
impl ShapeDisc {
    ...
    pub fn new(center: Vec2, pos2: Vec2) -> Option<ShapeDisc> {
...
impl ShapeDisc {
    ...
    fn get_vars(&self) -> ShapeDisc {
        self.clone()
    }

    fn set_vars(&mut self, shape_disc: &ShapeDisc) {
    ...
    fn move_controls(&mut self, pointer: &Pointer, _keys_states: KeysStates) -> bool {
        if self.radius_state.is_hs(HS::Select) {
